# Bank Deep Dive Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a single `.claude/commands/fdic-bank-deep-dive.md` skill file that produces a comprehensive 10-section narrative bank analysis report from public FDIC data.

**Architecture:** The entire deliverable is one markdown skill file — a prompt document that instructs an LLM how to execute a rigid workflow. No TypeScript, no build step. The skill follows the same structural conventions as the existing `/fdic-examiner-overlay` skill: YAML frontmatter with `allowed_tools`, then a step-by-step workflow body with section templates, narrative guidance, and degradation rules.

**Tech Stack:** Markdown skill file consumed by Claude Code's slash command system. References 10 MCP tools from the fdic-mcp-server.

**Spec:** `docs/plans/2026-03-29-fdic-bank-deep-dive-design.md`

---

## File Structure

- **Create:** `.claude/commands/fdic-bank-deep-dive.md` — the complete skill file (~350-450 lines)

That's it. Single file. The examiner overlay is the structural reference at `.claude/commands/fdic-examiner-overlay.md` (755 lines) — this skill will be shorter because it has no multi-turn input collection, no scoring math, and no structured JSON appendix.

---

### Task 1: Frontmatter and Skill Type Declaration

**Files:**
- Create: `.claude/commands/fdic-bank-deep-dive.md`

This task creates the file and writes the YAML frontmatter plus the skill type preamble. Everything after this task appends to this file.
...
- [ ] **Step 2: Verify the file exists and frontmatter parses correctly**

Run: `head -45 .claude/commands/fdic-bank-deep-dive.md`

Expected: The YAML frontmatter block with all 10 tools listed, followed by the skill type declaration.

- [ ] **Step 3: Commit**

```bash
git add .claude/commands/fdic-bank-deep-dive.md
git commit -m "feat: scaffold bank deep dive skill with frontmatter and type declaration"
```

---

### Task 2: Input Gate — Collect Input and Resolve CERT

**Files:**
- Modify: `.claude/commands/fdic-bank-deep-dive.md`

Append the input gate: Step 1 (collect input), Step 2 (resolve to CERT with tiered name search, candidate ranking, disambiguation, inactive fallback, and identity confirmation).

- [ ] **Step 1: Append Step 1 (Collect Input) to the skill file**
...
- [ ] **Step 2: Read the file to verify the input gate appended correctly**

Run: `wc -l .claude/commands/fdic-bank-deep-dive.md`
...
```bash
git add .claude/commands/fdic-bank-deep-dive.md
git commit -m "feat: add input gate with tiered name resolution and identity confirmation"
```

---

### Task 3: Input Gate — Validate Report Date

**Files:**
- Modify: `.claude/commands/fdic-bank-deep-dive.md`

Append Step 3: report date validation with quarter-end enforcement and the hard boundary rule.

- [ ] **Step 1: Append Step 3 (Validate Report Date) to the skill file**

Append after the Stop Conditions section:
...
```bash
git add .claude/commands/fdic-bank-deep-dive.md
git commit -m "feat: add report date validation with quarter-end enforcement and SOD year derivation"
```

---

### Task 4: Tool Chain Execution and Degradation Rules

**Files:**
- Modify: `.claude/commands/fdic-bank-deep-dive.md`

Append Step 4: the tool chain execution instructions, dependency tiers, degradation rules, and date basis transparency guidance.

- [ ] **Step 1: Append Step 4 (Execute Tool Chain) to the skill file**

Append after the Hard Boundary section:
...
```bash
git add .claude/commands/fdic-bank-deep-dive.md
git commit -m "feat: add tool chain execution with dependency tiers and degradation rules"
```

---

### Task 5: Report Envelope and Section Templates (Sections 1-5)

**Files:**
- Modify: `.claude/commands/fdic-bank-deep-dive.md`

Append Step 5 (render report): the report header, disclaimer, and section templates for sections 1 through 5.

- [ ] **Step 1: Append Step 5 opening, report header, and Sections 1-5**

Append after the Date Basis Transparency section:
...
```bash
git add .claude/commands/fdic-bank-deep-dive.md
git commit -m "feat: add report header and section templates 1-5"
```

---

### Task 6: Section Templates (Sections 6-10) and Report Footer

**Files:**
- Modify: `.claude/commands/fdic-bank-deep-dive.md`

Append section templates for sections 6 through 10, the report footer, and the optional save instructions.

- [ ] **Step 1: Append Sections 6-10, footer, and save instructions**
...
Expected: ~530-570 lines total.

- [ ] **Step 3: Commit**

```bash
git add .claude/commands/fdic-bank-deep-dive.md
git commit -m "feat: add section templates 6-10, report footer, and optional save"
```
...
**Files:**
- Read: `.claude/commands/fdic-bank-deep-dive.md` (full file)
- Read: `docs/plans/2026-03-29-fdic-bank-deep-dive-design.md` (spec)

Validate the complete skill file against the spec and fix any gaps.

- [ ] **Step 1: Read the complete skill file and verify against spec**

Read the full `.claude/commands/fdic-bank-deep-dive.md` file. Check each of these requirements from the spec:

1. **Frontmatter:** All 10 tools listed in `allowed_tools`? UUID note present?
2. **Skill type:** Declared as RIGID? Fixed section order stated?
3. **Input gate — Step 1:** Prompts for bank name or CERT + optional repdte?
4. **Input gate — Step 2:** Tiered name search (exact → loose → inactive fallback)? Always confirms identity? Disambiguation for 2-5 matches? "Narrow your search" for >5? Stop conditions listed?
5. **Input gate — Step 3:** Quarter-end validation? Suggests most recent completed quarter-end before invalid date? SOD year derivation rule? Hard boundary?
6. **Tool chain — Step 4:** Dependency tier table? Hard deps (1-3) stop report on failure? Soft/context deps (4-9) degrade gracefully? Three degradation states (full, structural immateriality, data unavailable)? Normalized error messages (no raw tool internals)? Date basis transparency rules?
7. **Report header:** Correct disclaimer text including FRED? "Monetary amounts in thousands"?
8. **Section 1:** Conditional holding company? Factual only, no interpretation?
9. **Section 2:** Proxy table with PCA flag? Risk signals by severity? Trend insights (not guaranteed per-component)?
10. **Section 3:** Standalone — no peer-relative language?
11. **Section 4:** Peer-set quality disclosure (thin cohort, auto-broadened)? n/a handling? No interpretation of n/a metrics?
12. **Section 5:** Threshold framing (screening indicators, not automatic adverse)? Structural immateriality for non-lenders?
13. **Section 6:** Distinguishes "data unavailable" from "zero brokered deposits"?
14. **Section 7:** Structural immateriality for minimal holdings? AFS/HTM unavailability noted?
15. **Section 8:** SOD reconciliation warning? Annual date basis?
16. **Section 9:** FRED fallback defined? No improvisation from undefined sources?
17. **Section 10:** Strengths, Risks, Outlook structure? Evidence-bounded rule?
18. **Report footer:** Correct source attribution? Not-official-regulatory disclaimer?
19. **Optional save:** Correct filename convention? Only on user request?

For any gap found, fix it immediately in the file.

- [ ] **Step 2: Verify the skill file is syntactically valid**

Run: `head -6 .claude/commands/fdic-bank-deep-dive.md` to confirm YAML frontmatter starts with `---`.
Run: `grep -c "^## " .claude/commands/fdic-bank-deep-dive.md` to confirm all 6 steps are present (Steps 1-6).
Run: `grep -c "^### Section" .claude/commands/fdic-bank-deep-dive.md` to confirm all 10 sections are present.

Expected: 6 step headers, 10 section headers.
...
```bash
git add .claude/commands/fdic-bank-deep-dive.md
git commit -m "fix: address spec validation gaps in bank deep dive skill"
```

If no fixes needed, skip this step.

- [ ] **Step 4: Run the skill against a real institution to validate**

Invoke `/fdic-bank-deep-dive` and provide CERT 3511 (Bank of America) to test the full workflow end-to-end. Verify:
- The input gate confirms the institution
- All 9 tools are called
- All 10 sections are rendered
- The report header and footer are present
- Date basis labels are correct in each section

This is a manual smoke test. If the skill produces a complete report, validation passes.

- [ ] **Step 5: Final commit**

```bash
git add .claude/commands/fdic-bank-deep-dive.md
git commit -m "feat: complete bank deep dive skill — validated against spec and smoke-tested"
```
